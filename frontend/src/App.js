...
export default function App() {
  ...
  return (
    <IntlProvider messages={getCurrentTranslation(locale)} locale={locale} defaultLocale='en'>
      <div className="ampay-container" style={pathname.startsWith("/receipt") ? {alignItems: 'unset'} : {}}>
        <Routes>
          <Route path="/" exact={true} element={<Home />} />
          <Route path="/checkout/:evseId" exact={true} element={<Checkout />} />
          <Route path="/charging/:evseId/:sessionId" exact={true} element={<Charging />} />
          <Route path="/receipt/:evseId/:sessionId" exact={true} element={<Receipt />} />
