...
export default function Charging(props) {
  ...
  const setSessionData = () => {
    axios.get(`checkouts/${sessionId}`).then(({data}) => {
      ...
      if(data.remote_request_status !== "Accepted") {
        ...
        if(transaction.transaction_start_time) {
          if(transaction.transaction_end_time) {
            ...
            const moment_start = moment.utc(transaction.transaction_start_time);
        ...
        return;
          ...
          if(sessionNotFoundRetryCounter <= 3) {
            ...
            setState({...state, status: 'error', statusMessage: 'charging.error.sessionnotfound' });
          }
      }
      else {
      ...
      // Set error on error
      setState({...state, status: 'error', statusMessage: e.response?.data?.detail});
    });
  }
  ...
  return(
    <div className="page-container" style={{height: '100%', padding: '15px'}}>
      ...
      <div className="checkout-top-container">
        ...
        <div>|</div>
        <div>max. { getConnectorPowerKw(locData.max_voltage, locData.max_amperage, locData.power_type)} kW</div>
      </div>

      {/* Charging Speed */}
      <div className="width-100 text-align-center" style={{marginTop: '40px', marginBottom: '40px'}}>
        ...
        </div>{/* Charging Speed END */}

        {/* Last update timestamp */}
        <div style={{fontSize: '16px', }}><i className="ri-refresh-fill"></i> {intl.formatMessage({id: "charging.lastupdate" })}: {state.timestamp}</div>

        {/* Refresh button only shown when waiting or charging */
          (state.status === 'charging' || state.status === 'waiting')  && (
            <Button onClick={onRefresh} style={{marginTop: '20px'}}><i className="ri-refresh-line"></i> {intl.formatMessage({id: "charging.refresh" })}</Button>
        )}
        ...
        (
          <>
            ...
            <div className="width-100 div-with-margin charging-info-block">
              <div>{intl.formatMessage({id: "charging.costs" })} ({intl.formatMessage({id: "checkout.inclvat" })}):</div>
              <div>
                <span>{(state.pricing?.total_costs_gross / 100).toFixed(2)} </span>
            ...
            {state.transaction_soc !== null && (  // only shown if we have an SoC
              <>
                ...
                />
                <span style={{fontSize: '16px'}}>{intl.formatMessage({id: "charging.soc.infotext" })}</span>
              </>
            )}
          ...
          <div className="text-align-center">
            ...
            {/* Don't worry */}
            <div className="width-100 div-with-margin charging-info-block" style={{justifyContent: 'center'}}>
            ...
            <Button color="primary" onClick={()=>navigate(`/checkout/${evseId}`)} ><i className="ri-flashlight-line"></i> {intl.formatMessage({id: "charging.button.again" })}</Button>
