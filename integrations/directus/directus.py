...
class DirectusIntegration(FileIntegration):
    def __init__(self, url: str, email: str = None, password: str = None, static_token: str = None):
        self.url: str = url
    ...
    def login(self):
        ...
        try:
            ...
            expires = response_payload['expires'] - REFRESH_TOKEN_REQUEST_BUFFER # in milliseconds
            timer = threading.Timer(expires /1000, self.refresh_auth)
            timer.start()
        except Exception as e:
            exception(" [CitrineOS - Directus] Processing error for Directus login: %r", e.__str__)
            raise e
    ...
    def refresh_auth(self):
        try:
            ...
            warning(" [CitrineOS - Directus] Error for Directus refresh auth: %r", e.__str__)
    ...
    def upload_file(self, file: BytesIO, mime_type: str, filename: str, filetitle: str) -> str:
        ...
        response = requests.post(request_url, data=data, files=files, auth=BearerAuth(self._token))
        response_payload = response.json().get("data")
